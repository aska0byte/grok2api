...
export function DegradeAccountsPanel({ softTPS, hardTPS, failClosed, minGenMs }: { softTPS?: number; hardTPS?: number; failClosed?: boolean; minGenMs?: number }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="overflow-hidden rounded-lg bg-card">
        <div className="flex flex-col gap-3 border-b px-4 py-4 sm:flex-row sm:items-start sm:justify-between sm:px-5">
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            <Button type="button" variant="secondary" size="sm" disabled={selectedRows.length === 0 || busy} title={t("qualityGuard.degrade.probeHelp")} onClick={() => setProbeOpen(true)}>
              <Gauge />{selectedRows.length ? t("qualityGuard.degrade.probeSelectedCount", { count: selectedRows.length }) : t("qualityGuard.degrade.probeSelected")}
            </Button>
          ...
          <Table className="min-w-[920px]">
            ...
            <TableBody>
              {rows.length === 0 ? (
                ...
                const canProbe = account.found;
                return (
                  <TableRow key={account.id} className={canProbe ? "cursor-pointer" : undefined} onClick={() => {
                    if (!canProbe) return;
                    setSelected((current) => {
                      const next = new Set(current);
                    ...
                    <TableCell className="px-3" onClick={(event) => event.stopPropagation()}>
                      <label className="flex size-8 cursor-pointer items-center justify-center">
                        <Checkbox disabled={!canProbe} checked={selected.has(account.id)} onCheckedChange={(checked) => setSelected((current) => {
                          const next = new Set(current);
      ...
      <QualityProbeDialog
        open={probeOpen}
        onOpenChange={(open) => { setProbeOpen(open); if (!open) setSelected(new Set()); }}
        mode="selected"
        selectedIds={selectedRows.map((account) => account.id)}
        onInvalidate={() => {
          void queryClient.invalidateQueries({ queryKey: ["quality-guard-degrade-accounts"] });
          void queryClient.invalidateQueries({ queryKey: ["accounts"] });
        }}
